...
// u8 Uart_Cnt = 0;
	...
	volatile u16 idata light1sflag = 0;
	 ...
	 bit idata startupflag = 0;
	 u8 idata startupflickerflag = 0;
	 u32 idata ledFlashingtimecount  =0;
	 bit idata ledFlashingtimeflag = 0;
	 //bit idata wait1flag = 0;
	 //bit idata wait2flag = 0;
	 /*
...
void set_var(void)
{
	...
	TH+=guc_Read_a[1];
	TH*=1000;
	if(TH<TH_LOW||TH>TH_HIGH)TH=RADIUS_6M;
  ...
  if(LIGHT_TH==0)LIGHT_TH=LIGHT_TH0;
	...
	if(DELAY_NUM==0||DELAY_NUM>MAX_DELAY)DELAY_NUM=10;
	...
	SWITCHflag2=  guc_Read_a[10]&0x01;
	
	//resetbtcnt = guc_Read_a[11]&0x01;
...
// 	
	...
	while(1)
	{
		WDTC |= 0x10;		//清看门狗
		bt_uart_service();
		if(startupflickerflag==1)
		{
			//startupflickerflag = 0;
			start();
			break;
		}
		if(startupflickerflag==2)
		{
			//startupflickerflag = 0;
			break;
		}
...
void TIMER1_Rpt(void) interrupt TIMER1_VECTOR
{
	...
	if(Exit_network_ontmincount>=250)
	{
	...
	ledFlashingtimecount++;
	if(ledFlashingtimecount>=180000)
	{
		ledFlashingtimecount = 0;
		ledFlashingtimeflag = 1;
	}
